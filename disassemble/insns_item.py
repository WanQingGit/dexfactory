# -- coding: utf-8 --
from disassemble.insns_type import op_map, KIND_PROTO
...
class InsnsItem(object):
    ...
    def __init__(self, bytes):
        ...
        self.op = op_map[bytes[0]]
        self.format_id = self.op.format
        self.kind = self.op.kind
        self.kind_x = None
        self.proto_x = None
    ...
    def decode(self):
        ...
        if op.format == '00x':
            ...
            op2 = op_map[bytes[0x00]]

            self.desc = '%s' % op2.formatOp()

        elif op.format == '12x' or op.format == '11n':
            ...
            if self.format_id == '12x':
                self.desc = '%s v%d, v%d' % (op2.formatOp(), A, B)
            elif self.format_id == '11n':
                self.desc = '%s v%d, #+%x' % (op2.formatOp(), A, B)
            ...
            if self.format_id == '11x':
                self.desc = '%s v%d' % (op2.formatOp(), AA)
            ...
            self.desc = '%s +%.4x' % (op2.formatOp(), AAAA)
            ...
            self.desc = '%s %.2d, %s' % (op2.formatOp(), AA, '%s')
            self.kind_x = BBBB
            ...
            if self.format_id == '22x':
                self.desc = '%s v%d, v%d' % (op2.formatOp(), AA, BBBB)
            elif self.format_id == '21t':
                self.desc = '%s v%d, +%.4x' % (op2.formatOp(), AA, BBBB)
            elif self.format_id == '21s':
                self.desc = '%s v%d, #+%.4x' % (op2.formatOp(), AA, BBBB)
            elif self.format_id == '21h':
                self.desc = '%s v%d, #+%.4x0000' % (op2.formatOp(), AA, BBBB)
            elif self.format_id == '21c':
                self.desc = '%s v%d, %s' % (op2.formatOp(), AA, '%s')
                self.kind_x = BBBB
            ...
            if self.format_id == '23x':
                self.desc = '%s v%d, v%d, v%x' % (op2.formatOp(), AA, BB, CC)
            elif self.format_id == '22b':
                self.desc = '%s v%d, v%d, #+%.2x' % (op2.formatOp(), AA, BB, CC)
            ...
            if self.format_id == '22t':
                self.desc = '%s v%d, v%d, +%.4x' % (op2.formatOp(), A, B, CCCC)
            elif self.format_id == '22s':
                self.desc = '%s v%d, v%d, #+%.4x' % (op2.formatOp(), A, B, CCCC)
            elif self.format_id == '22c':
                self.desc = '%s v%d, v%d, %s' % (op2.formatOp(), A, B, '%s')
                self.kind_x = CCCC
            ...
            self.desc = '%s +%.8x' % (op2.formatOp(), AAAAAAAA)
            ...
            self.desc = '%s v%d, v%d' % (op.formatOp(), AAAA, BBBB)
            ...
            if self.format_id == '31i':
                self.desc = '%s v%d, #+%.8x' % (op2.formatOp(), AA, BBBBBBBB)
            ...
            if A == 0:
                self.desc = '%s {}, %s' % (op2.formatOp(), '%s')
                self.kind_x = BBBB
            elif A == 1:
                self.desc = '%s {v%d}, %s' % (op2.formatOp(), C, '%s')
                self.kind_x = BBBB
            elif A == 2:
                self.desc = '%s {v%d, v%d}, %s' % (op2.formatOp(), C, D, '%s')
                self.kind_x = BBBB
            elif A == 3:
                self.desc = '%s {v%d, v%d, v%d}, %s' % (op2.formatOp(), C, D, E, '%s')
                self.kind_x = BBBB
            elif A == 4:
                self.desc = '%s {v%d, v%d, v%d, v%d}, %s' % (op2.formatOp(), C, D, E, F, '%s')
                self.kind_x = BBBB
            elif A == 5:
                self.desc = '%s {v%d, v%d, v%d, v%d, v%d}, %s' % (op2.formatOp(), C, D, E, F, G, '%s')
                self.kind_x = BBBB
            ...
            self.desc = '%s {v%d ... v%d} %s' % (op.formatOp(), CCCC, NNNN, '%s')
            self.kind_x = BBBB
            ...
            if A == 1:
                self.desc = '%s {v%d}, %s, %s' % (op2.formatOp(), C, '%s', '%s')
                self.kind_x = BBBB
                self.proto_x = HHHH
            elif A == 2:
                self.desc = '%s {v%d, v%d}, %s, %s' % (op2.formatOp(), C, D, '%s', '%s')
                self.kind_x = BBBB
                self.proto_x = HHHH
            elif A == 3:
                self.desc = '%s {v%d, v%d, v%d}, %s, %s' % (op2.formatOp(), C, D, E, '%s', '%s')
                self.kind_x = BBBB
                self.proto_x = HHHH
            elif A == 4:
                self.desc = '%s {v%d, v%d, v%d, v%d}, %s, %s' % (op2.formatOp(), C, D, E, F, '%s', '%s')
                self.kind_x = BBBB
                self.proto_x = HHHH
            elif A == 5:
                self.desc = '%s {v%d, v%d, v%d, v%d, v%d}, %s, %s' % (op2.formatOp(), C, D, E, F, G, '%s', '%s')
                self.kind_x = BBBB
                self.proto_x = HHHH
            ...
            self.desc = '%s> {v%d ... v%d}, %s, %s' % (op2.formatOp(), CCCC, NNNN, '%s', '%s')
            self.kind_x = BBBB
            self.proto_x = HHHH
            ...
            self.desc = '%s v%d, #+%.16x' % (op2.formatOp(), AA, BBBBBBBBBBBBBBBB)
